...
def render(app: Dash, data: pd.DataFrame) -> html.Div:
    ...
    def update_genre_charts(selected_genre, content_type):
        ...
        if len(filtered_data) > 0:
            grouped_timeline = filtered_data.groupby(
                ["release_year", "type"], observed=True
            ).size()
            ...
            timeline_fig = px.bar(title="No timeline data available")
            timeline_fig.update_layout(
                plot_bgcolor=t["card_bg"],
                paper_bgcolor=t["card_bg"],
                title_font_size=16,
        ...
        if len(filtered_data) > 0:
            grouped_heatmap = filtered_data.groupby(
                ["country_primary", "genre"], observed=True
            ).size()
            ...
            top_countries = (
                filtered_data["country_primary"].value_counts().head(10).index
            )
            heatmap_data = heatmap_data.loc[
                heatmap_data["country_primary"].isin(top_countries)
            ]

            heatmap_pivot = heatmap_data.pivot(
                index="country_primary", columns="genre", values="count"
            ).fillna(0)
            ...
            heatmap_fig = px.imshow([[0]], title="No heatmap data available")
            heatmap_fig.update_layout(
                plot_bgcolor=t["card_bg"],
                paper_bgcolor=t["card_bg"],
                title_font_size=16,
                    ...
                    html.Div(
                        [
                            ...
                            dcc.Dropdown(
                                ...
                                + [
                                    {"label": str(genre), "value": str(genre)}
